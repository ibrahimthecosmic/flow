...
macro_rules! v8_slice_from {
  (tensor::<$type:ident>($tensor:expr)) => {{
    ...
    if n_detections == 0 {
      ...
      // SAFETY: the empty range 0..0 is always within the (empty) backing
      // store created just above.
      ...
      // SAFETY: tensor_ptr is valid for buffer_len bytes and stays alive
      // until drop_tensor releases the Rc leaked above, which V8 calls when
      // the backing store is garbage-collected.
      ...
      // SAFETY: 0..buffer_len is exactly the extent of the backing store
      // created above.
      let buffer_slice = unsafe {
...
#[allow(
  dead_code,
  reason = "serde remote-derive helper (https://serde.rs/remote-derive.html), never constructed directly"
)]
