...
// SAFETY: TimerId wraps a kernel POSIX timer handle (timer_t), not a pointer
// to thread-local memory; the handle stays valid on any thread until
// timer_delete.
#[cfg(target_os = "linux")]
...
impl CPUTimer {
  ...
  pub fn new() -> Result<Self, Error> {
    ...
    // SAFETY: an all-zero byte pattern is a valid libc::sigevent; the
    // relevant fields are initialized below.
    ...
    // SAFETY: sigev and tid.0 are valid for the duration of the call;
    // timer_create writes the new timer handle into tid.0.
    if unsafe {
  ...
  pub fn reset(&self, initial_expiry: u64, interval: u64) -> Result<(), Error> {
    ...
    // SAFETY: an all-zero byte pattern is a valid libc::itimerspec; the
    ...
    // SAFETY: timer.tid.0 is a live timer handle from timer_create and
    // tmspec is a valid itimerspec; a null old_value is allowed.
    if unsafe {
...
pub fn get_thread_time() -> Result<i64, Error> {
  ...
  // SAFETY: time is a valid, writable libc::timespec out-pointer.
    ...
    .spawn(|| {
      ...
      let sig_receiver_handle = rt.spawn(async move {
        ...
        while let Some(siginfo) = signals.next().await {
          // SAFETY: this SIGALRM was queued by a POSIX timer created with
          // SIGEV_SIGNAL, so reading the si_value union is defined and
          // sival_ptr carries the timer id stored in sigev_value.
