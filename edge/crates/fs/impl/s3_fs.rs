...
pub struct S3Fs {
  ...
  #[allow(
    unused,
    reason = "keeps the config (incl. credentials) alive as long as the client built from it"
  )]
...
impl deno_fs::FileSystem for S3Fs {
  ...
  fn read_dir_sync(&self, path: &CheckedPath) -> FsResult<Vec<FsDirEntry>> {
    std::thread::scope(|s| {
      ...
      s.spawn(move || {
        rt::IO_RT.block_on(async move {
          // read_dir_async now returns a streaming FsReadDirRc; drain it.
          let rd = self
...
impl FileBackedMmapBuffer {
  fn new() -> Result<Self, anyhow::Error> {
    ...
    // SAFETY: the mapping was just created with length raw.len() and stays
    // alive in `raw` alongside the cursor, which has exclusive access to it.
    ...
    self.tasks.push(
      tokio::task::spawn({
        ...
        async move {
          ...
          // SAFETY: the cursor position is how many bytes were written into
          // the mapping, so the range is initialized and within bounds; `buf`
          // keeps the mapping alive while `data` is in use.
          let data = unsafe {
    ...
    match self {
      ...
      Self::PutObject => {
        client
          ...
          .key(key)
          .body(ByteStream::new(
            // SAFETY: total_written bytes were written into the mapping, so
            // the range is initialized and within bounds; the slice is
            // copied into the ByteStream before `state` is dropped.
            unsafe {
...
impl deno_io::fs::File for S3Object {
  fn read_sync(self: Rc<Self>, buf: &mut [u8]) -> FsResult<usize> {
    ...
    std::thread::scope(|s| {
      s.spawn(move || {
        rt::IO_RT.block_on(async move {
          // SAFETY: reconstructs the Rc leaked by into_ptr above, on the scoped
          // thread the parent is joining on, so it is reclaimed exactly once.
