...
mod flow_startup_snapshot {
  ...
  /// The worker extension set, used both for the (experimental) worker
  /// snapshot and for embedding extension sources into the binary.
  ///
  ...
  /// Every extension source file (eager `js`/`esm` and `lazy_loaded_*`) whose
  /// bytes live on disk at build time (`LoadedFromFsDuringSnapshot`), as
  /// `(specifier, absolute path)`. These are the sources that would otherwise
  /// be read from the build machine's filesystem at worker-creation time — the
  /// cause of the "panics on any release build" bug when the binary runs on a
  /// different machine than it was built on. We embed them into the binary
  /// instead (see `create_embedded_ext_sources`).
  fn collect_fs_extension_files(
    extensions: &[Extension],
  ) -> Vec<(String, PathBuf)> {
    fn push(out: &mut Vec<(String, PathBuf)>, files: &[ExtensionFileSource]) {
      for file in files {
        #[allow(deprecated, reason = "matching variant used by the ext macro")]
        if let ExtensionFileSourceCode::LoadedFromFsDuringSnapshot(p) =
          &file.code
        {
          out.push((file.specifier.to_string(), PathBuf::from(p)));
        }
      }
    }
    ...
    let mut out = Vec::new();
    for ext in extensions {
      push(&mut out, &ext.js_files);
      push(&mut out, &ext.esm_files);
      push(&mut out, &ext.lazy_loaded_js_files);
      push(&mut out, &ext.lazy_loaded_esm_files);
    }
  ...
  /// (a build-machine path) to `Computed` (in-memory), so worker creation
  /// never touches the filesystem for extension sources.
  ///
  /// Raw (un-transpiled) source is embedded on purpose: the runtime applies
  /// its own extension transpiler to these sources exactly as it does for the
  /// disk-read path today, so behavior is identical — only the byte source
  /// changes (memory vs disk).
  pub fn create_embedded_ext_sources(embedded_path: PathBuf) {
    let extensions = worker_extensions();
    let files = collect_fs_extension_files(&extensions);

    let mut f = std::fs::File::create(&embedded_path).unwrap();
    writeln!(
      f,
      "// @generated by edge/crates/base/build.rs - do not edit.\n\n\
       pub static EMBEDDED_EXT_SOURCES: &[(&str, &str)] = &["
    )
    .unwrap();
  ...
  pub fn create_runtime_snapshot(
    ...
    let extensions: Vec<Extension> = worker_extensions();
...
fn main() {
  ...
  // The worker startup snapshot is never LOADED (see src/snapshot.rs: one
  // snapshot blob per process), so worker isolates always boot fresh and need
  // every extension source at runtime. Embed those sources into the binary so
  // they are served from memory rather than read from the build machine's
  // filesystem — the latter panics on any binary run on a different machine
  // than it was built on. Generated unconditionally, since it is the runtime
  // source of truth in both build modes.
  let embedded_path = o.join("EMBEDDED_EXT_SOURCES.rs");
  flow_startup_snapshot::create_embedded_ext_sources(embedded_path);
}
