  ...
  pub async fn listen(&mut self) -> Result<Option<SignumOrExitCode>, Error> {
    ...
    loop {
      ...
      tokio::select! {
        ...
        _ = async move {
          ...
          debug!("exit: termination token resolved");
        ...
        _ = &mut main_worker_cancel_fut => {
          debug!("exit: main worker has been destroyed");
        ...
        signum = &mut terminate_signal_fut => {
          debug!("exit: shutdown signal received: {}", signum);
        ...
        _ = signal::ctrl_c() => {
          debug!("exit: interrupt signal received");
    ...
    if loop_state.is_main_worker_destroyed() {
      ...
      let Some(err) = self.main_worker_surface.exit.error().await else {
        debug!("main worker destroyed with no error info");
        return Ok(ret);
      };

      error!("main worker error: {:?}", err);
      error!("{err:?}");
    ...
    if !loop_state.is_interrupted() && graceful_exit_deadline_sec > 0 {
      ...
      let wait_fut = async move {
        ...
        loop {
          ...
          trace!(
            "io: {}, received: {}, handled: {}",
            active_io, received_request_count, handled_request_count
          );
    ...
    {
      warn!(
        "runtime exits immediately since the graceful exit feature has been disabled"
      );
    }
