...
mod internal {
  ...
  pub fn home_dir() -> Option<PathBuf> {
    std::env::var_os("HOME")
      ...
      .or_else(|| {
        // SAFETY: `fallback` is the getpwuid_r pattern from std's home_dir;
        // every pointer passed is valid for the duration of the call and
        // pw_dir is a valid C string when getpwuid_r succeeds with a
        // non-null result.
        unsafe { fallback() }
      })
      .map(PathBuf::from)
