...
impl WorkerBuilder {
  ...
  pub(crate) fn build(self) -> Result<Worker, Error> {
    ...
    let worker_naming_fn = worker_naming_fn.unwrap_or(Box::new(|uuid| {
      uuid
        .map(|it| format!("sb-iso-{:?}", it))
        .unwrap_or("isolate-worker-unknown".to_string())
    }));

    let worker_key = conf.key;
    let worker_name = worker_naming_fn(worker_key);
    let worker_cancel_token = conf.cancel.clone();
    let worker_pool_msg_tx = conf.pool_msg_tx.clone();
    let worker_events_msg_tx = conf.events_msg_tx.clone();
...
impl Worker {
  /// Start the worker on a dedicated OS thread. Returns the thread/isolate
  /// handles, or `None` when the thread could not be stood up (the boot error
  /// is delivered through `booter_signal`).
  ///
  /// The dedicated thread eliminates the need for v8::Locker by ensuring the
  /// isolate never migrates threads.
  pub async fn start(
    self,
    eager_module_init: bool,
    booter_signal: oneshot::Sender<Result<CancellationToken, Error>>,
    exit: WorkerExit,
    ...
    // V8 requires substantial stack space for complex operations like npm
    // module loading. The default stack size (512KB on macOS) is insufficient
    // and causes SIGBUS/SIGSEGV crashes in v8threads.cc during thread
    // initialization. 8MB is the same as the default main thread stack size on
    // macOS and matches what Deno uses for its worker threads.
    ...
    let thread_result = std::thread::Builder::new()
      ...
      .spawn(move || {
        ...
        thread_utils::block_on_local(&rt, async move {
          ...
          let _ = booter_signal.send(Ok(new_runtime.drop_token.clone()));
          ...
          let service_path = new_runtime.conf.service_path.clone();
...
pub struct WorkerSurface {
  ...
  /// Thread handles for the worker's dedicated thread.
