#![allow(
  ...
  reason = "single-threaded tests; Arc-wrapped values never cross threads"
)]
#![allow(
  clippy::async_yields_async,
  reason = "test helpers intentionally return futures to be awaited later"
)]
...
impl TlsExt for Option<Tls> {
  ...
  fn schema(&self) -> &'static str {
    if self.is_some() { "https" } else { "http" }
  }
